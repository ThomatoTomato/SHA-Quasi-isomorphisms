...
\begin{document}
    ...
    \subsection{Coalgebras}
            ...
            \begin{proposition}[Cofree tensor coalgebra]\label{prop: cofree-tensor}
                The tensor coalgebra is the cofree conilpotent coalgebra over the category of $\mathbb{K}$-modules\todo{Fiks dette?}, i.e. for any $\mathbb{K}$-module $V$ and any conilpotent coalgebra $C$ there is a natural isomorphism $\tt{Hom}_{\mathbb{K}}(C, V)\simeq \tt{CoAlg}_{\mathbb{K}, conil}(C, T^c(V))$.
            \end{proposition}
    ...
    \subsection{Electric Circuits}
            ...
            For convenience we will let $\mathcal{C} = \tt{Mod}_\mathbb{K}$. This is a closed symmetric monoidal category, with $\otimes_\mathbb{K}$ as $\otimes$. Recall that an algebra is a $\mathbb{K}$-module $A$ together with maps $(\cdot_A) : A \otimes A \rightarrow A$ and $1_A : \mathbb{K} \rightarrow A$. These morphisms will be denoted electrically as the diagrams below.
            \begin{center}
                \begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm, thick, op/.style={circle, draw, scale=0.75}, scale=0.7]
            ...
            We write the electric laws for an algebra as how one would write equations. Associativity and unitality then becomes as follows.
            \begin{center}
                \begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm, thick, op/.style={circle, draw, scale=0.75}, scale=0.7]
            ...
            Dually, given a coalgebra $C$ we make the same notation, but everything will be turned upside down. The maps $\Delta_C : C \rightarrow C \otimes C$ and $\varepsilon_C : C \rightarrow \mathbb{K}$ will be denoted as the following circuits.
            \begin{center}
                \begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm, thick, op/.style={circle, draw, scale=0.75}, scale=0.7]
                ...
                \end{tikzpicture}
            \end{center}
            The electric laws for $C$ becomes the following diagrams.
            \begin{center}
                \begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm, thick, op/.style={circle, draw, scale=0.75}, scale=0.7]
    ...
    \subsection{Derivations and DG-Algebras}
            ...
            \begin{proposition}\label{prop: tensor-derivation}
                ...
                Let $N$ be a $T^c(V)$-cobimodule. A $\mathbb{K}$-linear morphism $g:M\rightarrow V$ uniquely determines a coderivation $d_g^c:N\rightarrow T^c(V)$, i.e. there is an isomorphism $\tt{Hom}_{\mathbb{K}}(N,V)\simeq \tt{Coder}(N,T^c(V))$.
            \end{proposition}
            ...
            When there is no ambiguity, we will start to adopt writing the differential in electric circuits as a triangle.
            ...
            \begin{proof}
                ...
                \begin{multline*}
                    (b_1 \circ a_1) \otimes \cdots \otimes (b_n \circ a_n) = (-1)^{s'}((b_1 \otimes \cdots \otimes b_{n-1}) \circ (a_1 \otimes \cdots \otimes a_{n-1})) \otimes (b_n \circ a_n) \\ = (-1)^{s' + |b_n|(\sum_{i = 1}^{n-1}|a_i|)}(b_1 \otimes \cdots \otimes b_n) \circ (a_1 \otimes \cdots a_n)
                \end{multline*}
            \end{proof}
            ...
            \begin{example}[Cone of a chain map]
                ...
                \end{center}
            \end{example}
            \begin{example}[Simplical cochain complexes]
                Let $A : \Delta^{op} \rightarrow \tt{Mod}_\mathbb{K}$ be a simplical $\mathbb{K}$-module. Taking the alternating sum of the face maps of the same degrees gives us a differential on the graded $\mathbb{K}$-module $A^*$. 
            \end{example}
            \begin{example}[Singular chain complex with $\mathbb{K}$-coefficents]
                Let $M$ be a topological space. There is a simplical set defined as $Sing(M) = \tt{Top}(\Delta^{\argument},M) : \Delta^{op} \rightarrow \tt{Set}$. Here $\Delta^{[n]}$ in Top refers to the topological standard $n$-simplex. We get a simplical $\mathbb{K}$-module by creating the free one, $\mathbb{K}Sing(M)$. By the above example this in turns defines a chain complex in $\tt{Mod}_\mathbb{K}$.
            \end{example}
            ...
            \end{align*}
            Thus $\tt{Hom}_\mathbb{K}^\bullet(A^\bullet, B^\bullet) = (\tt{Hom}_\mathbb{K}^*(A^\bullet, B^\bullet), \partial)$ is a cochain complex. We endow $Mod_\mathbb{K}^\bullet$ with these hom-objects. In an electric diagram we write $\partial f$ as a sum of circuits.
            ...
            \end{align*}
            All of the structure morphisms for a closed symmetric monoidal category are inherited from $\tt{Mod}_\mathbb{K}^*$. This also means that $\tt{Mod}_\mathbb{K}^\bullet$ employs the same electric circuits as $\tt{Mod}_\mathbb{K}^*$.
            ...
            \begin{align*}
                \mathbb{K}^\bullet[n]\otimes_\mathbb{K} M^\bullet \simeq M^\bullet[n]\tt{.}
            \end{align*}
            ...
            Dually, $C^{\bullet}$ is a differential graded coalgebra if
            \begin{itemize}
    ...
    \subsection{Convolution Algebras}
            ...
            \begin{proposition}\label{prop: convolution to endomorphism}
                $d_{(\argument)}:\tt{Hom}_\mathbb{K}(C,A)\rightarrow End(C\otimes_\mathbb{K}A)$ is a morphism of algebras. Moreover, if $\alpha\star \alpha = 0$, then $d_\alpha^2 = 0$.
            \end{proposition}
            ...
            Suppose that $C$ and $A$ are differential graded (co)algebras. We want to expect that the differential $\partial$ makes $\tt{Hom}_\mathbb{K}^*(C,A)$ into a dg-algebra.
