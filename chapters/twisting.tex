...
\begin{document}
    ...
    \subsection{Bar and Cobar Construction}
            ...
            Let $\mathcal{A}$ be an abelian category. To each semi-simplical object $M:(\Delta^+)^{op}\rightarrow \mathcal{A}$ there is an associated chain complex $M^\bullet$. Let $M^{-i} = M[i]$ with differential $d_M^{-n} = \sum_{i=1}^{n+1} (-1)^{i-1}d^i$. This differential is well-defined by simplicial identity $1$.
            \begin{center}
                \begin{tikzcd}
