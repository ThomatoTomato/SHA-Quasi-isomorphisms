...
\begin{document}
    \section{Algebras, Coalgebras and Twisting Morphisms}
        ...
        \subsection{Bar and Cobar Construction}
            ...
            If $A$ is an augmented algebra, then there is an $A$-linear morphism $\varepsilon_\mathbb{K}: A \rightarrow \mathbb{K}$. The augmented canonical free-resolution of $\mathbb{K}$ is the Hochschild complex for $A$, i.e. $A\otimes_\mathbb{K}U_\mathbb{K}^\bullet \rightarrow \mathbb{K}$ is
            \begin{center}
                \begin{tikzcd}
                    ... \ar[]{r}[]{} & A\otimes_\mathbb{K}A \ar[]{r}[]{\substack{\varepsilon_{A} \\ -A\otimes_\mathbb{K}\varepsilon_\mathbb{K}}} & A \ar[]{r}[]{\varepsilon_\mathbb{K}} & \mathbb{K} \ar[]{r}[]{0} & ...
                \end{tikzcd}
            \end{center} 

            The bar construction $BA$ is the restriction of the augmented Hochschild complex to the augmentation ideal of $A$. The differential is called $d_2$.
            \begin{center}
                \begin{tikzcd}
                    
                    ... \ar[]{r}[]{} & \bar{A}^{\otimes 3} \ar[]{r}[]{\substack{\varepsilon_{A\otimes A} \\ -A\otimes_\mathbb{K}\varepsilon_A}} & \bar{A}^{\otimes 2} \ar[]{r}[]{\varepsilon_A} & \bar{A} \ar[]{r}[]{0} & \mathbb{K} \ar[]{r}[]{0} & 0 \ar[]{r}[]{} & ...
                \end{tikzcd}
            \end{center}
            
            We want to realize the bar construction as a conilpotent coalgebra. Notice that as a graded vector space, $BA$ is isomorphic to the cofree tensor coalgebra of the shifted augmentation ideal $T^c(\bar{A}[1])$. The multiplication has an associated morphism $\mu:\bar{A}[1]^{\otimes 2}\rightarrow \bar{A}[1]$ which is of degree $-1$. By Koszul sign rule we are forced to let $\mu(ab) = (-1)^{|a|}\varepsilon_A(ab)$, since $|a| = 0$ we obtain $\mu(ab) = \varepsilon_A(ab)$. 

            By proposition \ref{prop: tensor-derivation}, we know that there is a coderivation $d_2$ defined by extending $\mu$. This map is defined levelvise as 
            \begin{multline*}
                d_2^n (a_1...a_n) = \sum_{i=1}^n \mu_{(n)}^{i}(a_1...a_n) \\
                = \sum_{i=1}^n(-1)^{i-1 + |a_1| + |a_2| + ... + |a_i|}a_1...\mu(a_ia_{i+1})...a_n = \sum_{i=1}^n(-1)^{i-1}a_1...\mu(a_ia_{i+1})...a_n. 
            \end{multline*}
            We see that both $d_2$ are the same map, so we obtain the following result.
