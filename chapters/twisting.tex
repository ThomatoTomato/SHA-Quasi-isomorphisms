...
\begin{document}
    ...
    \section{Strongly Homotopy Associative Algebras and Coalgebras}
        ...
        \begin{lemma}\label{lem: coderivation-is-diff?}
            ...
            \begin{align*}
                (\text{rel}_n)\qquad & \sum_{p+q+r = n}m_{p+1+r}\circ_{p+1}m_q = 0
            \end{align*}
        \end{lemma}
        ...
        \begin{proof}
            ...
            For $m^2$ we write it's n'th component as $m^2_n$. Observe the following:
            \begin{align*}
        ...
        \begin{proof}[Proof of proposition \ref{prop: A-infinity def}]
            ...
            Since $m^2 = 0$, we know by the above lemma that the n'th component of $m^2$ is:
            \begin{multline*}
                \sum_{p + q + r = n}\widetilde{m}_{p+1+r}\circ_{p+1}\widetilde{m}_q \\
                = \sum_{p + q + r = n}sm_{p+1+r}s^{-\otimes (p+1+r)}\circ_{p+1}sm_qs^{-\otimes q} = \sum_{p + q + r = n}(-1)^{pq+r}sm_{p+1+r}\circ_{p+1}m_q s^{-\otimes n} = 0
            \end{multline*}
        \end{proof}
\end{document}