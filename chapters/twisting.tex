...
\begin{document}
        ...
        \begin{center}
            \begin{tikzcd}
                \tt{Alg}_{\mathbb{K}, +}^\bullet \ar[yshift = -1ex]{r}[below]{B} \ar[phantom]{r}{\bot} & \tt{Coalg}_{\mathbb{K},\tt{conil}}^\bullet \ar[yshift = 1ex]{l}[above]{\Omega}
            \end{tikzcd}
        \end{center}
    ...
    \subsection{Derivations and DG-Algebras}
            ...
            Following Kelly \cite{Kelly05}, we define a symmetric monoidal structure on this category. We give a braiding on homogenous elementary tensors as
            \begin{align*}
            ...
            Since $\tt{Mod}_\mathbb{K}^*$ is a closed symmetric monoidal category, it admits electronic circuits. Thus the previous results we have proved by electronic circuits also apply to this category, as the proof is identical in this language. One should note that the specific implementation may differ as vertical braiding works differently. The application of two homogenous morphisms $f : A \rightarrow A'$ and $g : B \rightarrow B'$ on elements $a \in A$ and $b\in B$ on tensors is defined as
            \begin{align*}
            ...
            A cochain complex $N^\bullet$ is said to be contractible if $id_N$ is null-homotopic. Then it follows for any other cochain complex $M^\bullet$ that $H^0\tt{Hom}_\mathbb{K}^\bullet(M^\bullet, N^\bullet) \simeq 0$.

            The shift functor $\argument [n] : \tt{Mod}_\mathbb{K}^\bullet \rightarrow \tt{Mod}_\mathbb{K}^\bullet$ is defined on cochains $M^\bullet$ as
            \begin{align*}
            ...
            \begin{definition}
                Suppose there are morphisms $f,g: A^\bullet \rightarrow B^\bullet$. We say that $h : A \rightarrow B$ is an $(f,g)$-derivation if $|h| = -1$ and $h \circ (\cdot_A) = (\cdot_A)\circ (f \otimes h + g \otimes h)$.
            \end{definition}
            ...
            With this definition, the categories $\tt{Mod}_\mathbb{K}$ where $\mathbb{K}$ is considered as a cochain complex, and the category $\tt{Mod}_\mathbb{K}^\bullet$ is the same category because a cochain complex already satisfies the first two bullet points by definition. Being a $\mathbb{K}^\bullet$-derivation is a trivial condition, so every map meets this.
    ...
    \subsection{Convolution Algebras}
            ...
            \begin{proof}
                We know that $(\tt{Hom}_\mathbb{K}^*(C,A),\star)$ is a convolution algebra and that $(\tt{Hom}_\mathbb{K}^*(C,A),\partial)$ is a cochain complex. It remains to verify that the differential is compatible with the multiplication, i.e., $\partial(f\star g) = \partial{f}\star g + (-1)^{|f|}f\star\partial{g}$.
    ...
    \subsection{Twisting Morphisms}
            ...
            \begin{corollary}
                If $\alpha: C\rightarrow A$ is a twisting morphism, then $(C\otimes_\mathbb{K}A, d_\alpha^\bullet)$ is a cochain complex which is also a left $C$-comodule and a right $A$-module. We call this the right twisted tensor product, denoted as $C\otimes_\alpha A$.
            \end{corollary}
    ...
    \subsection{Bar and Cobar Construction}
            ...
            % \begin{definition}
            %     Suppose that $\mathcal{A}$ is an abelian category. Let $W:\mathcal{A}\rightarrow\mathcal{A}$ be a comonad and $A$ and object of $\mathcal{A}$. The canonical $W$-projective resolution of $A$ is the cochain complex $W^\bullet_A$ together with an augmentation $\varepsilon_A: W^\bullet_A \rightarrow A$.

            %     Dually, suppose that $M:\mathcal{A}\rightarrow\mathcal{A}$ is a monad. A's canonical $M$-injective resolution is the cochain complex $M_A^\bullet$ together with an augmentation $\eta_A: A\rightarrow MA$.
            % \end{definition}
            ...
            \begin{proposition}
                Let $A$ be an augmented dg-algebra. The bar complex $BA$ is the total associated cochain complex of the augmentation ideal $\overline{A}$. $(BA, d_{BA}^\bullet)$ is the cofree conilpotent coalgebra equipped with $d_{BA}^\bullet = d_1 + d_2$ as coderivation.
            \end{proposition}
            ...
            \begin{proposition}
                Let $C$ be a coaugmented dg-coalgebra. The cobar complex $\Omega C$ is the total associated cochain complex of the suspended coaugmentation quotient $\overline{C}[-1]$. $(\Omega C, d_{\Omega C})$ is the free algebra equipped with the differential $d_{\Omega C} = d_1 + d_2$ as derivation.
            \end{proposition}
    ...
    \subsection{SHA-Algebras}
        ...
        A homotopy between two $A_\infty$-algebras is a homotopy between the dg-coalgebras they define. We may trace this definition back along the quasi-inverse of the bar construction to get a new definition in terms of many morphisms. 
        \begin{definition}    
            Let $f,g: A\rightsquigarrow B$ be two $\infty$-morphisms, we say that $f\sim g$ are homotopic if there is a collection of morphisms $h_n : A^{\otimes n} \rightarrow B$ of degree $-n$ such that the following relations are satisfied for any $n \geq 1$:
            \begin{multline*}
            ...
            \end{multline*}
            $s$ is some constant depending on $t,r$, and $k$, which is calculable with Koszul's sign rule. More specific details may be found in \cite{LefevreHasegawa03}. 
        \end{definition}
        One may observe that this definition of homotopy is exactly the same as requiring that the morphisms $Bf$ and $Bg$ are homotopic by a $(Bf, Bg)$-coderivation $Bh$. 
        
        If we have morphisms of algebras $f,g:A\rightarrow~A'$ such that they are homotopic, then the $(f,g)$-derivation $h:A\rightarrow~A'$ defines a homotopy between $f$ and $g$ if we consider them as strict $\infty$-morphisms. The relations for when $n = 2$ describes the property of being an $(f,g)$-derivation whenever $f_2$ and $g_2$ are both $0$, which is the case by strictness. The higher relations will be trivially satisfied in this case. Thus, we may see that the bar construction maps $(f,g)$-derivations to $(f,g)$-coderivations.
