...
\begin{document}
    ...
    \section{Strongly Homotopy Associative Algebras and Coalgebras}
        ...
        \begin{example}
            An associative dg-algebra is an $A_\infty$ algebra with trivial higher morphisms.
        \end{example}
        ...
        Let $Alg_\infty$ denote the category of $A_\infty$-algebras. The morphisms in this category are the $\infty$-morphisms. Observe that the bar construction $B : Alg_\infty \rightarrow ConilCoalg^\bullet$ is a fully fatihful functor, identifying $Alg_\infty$ as a subcategory of $ConilCoalg^\bullet$.

        Dual to $A\infty$-algebras we got $A_\infty$-coalgebras. This will be th

        \begin{definition}
            A graded vector space $C$ is called an $A_\infty$-coalgebra if it is a dg-algebra of the form $(\bar{T}(B[-1]), d)$ where $d$ is a derivation of degree $-1$. Dually, this is equivalent to a chain complex $(B, d^1)$, where $d^1$ is of degree $1$, and together with morphisms $d^n : B \rightarrow B^{\otimes n}$. The morphism should satisfy the relations:
            \begin{align*}
