...
\begin{document}
    ...
    \subsection{A Model Structure on DG-Coalgebras}
            ...
            \begin{lemma}[Key lemma]\label{lem: tech-fac}
                ...
                \begin{center}
                    \begin{tikzcd}
                        BA\prod_{B\Omega D}D \ar[]{d}[]{\pi} \ar[]{r}[]{} \ar[phantom]{rd}[near start]{\ulcorner} & D \ar[]{d}[]{\eta_D} \\
            ...
            \begin{proof}
                ...
                \begin{align*}
                    & F_n(BA\ast_{B\Omega D}D) = F_n(T^c(\overline{K}[1]\oplus \overline{D})) = \bigoplus_{k=0}^\infty\,\,\sum_{\substack{n_1+\cdots+n_k \\ \leq n}}\,\,\bigotimes_{i = 1}^k\,(\overline{K}[1]\oplus Fr_{n_i}\overline{D})\tt{ and} \\
                ...
                \end{align*}
                Here $Fr$ and $\widetilde{Fr}$ respectively refers to the coradical and induced coradical filtration. This filtration is made to be agnostic towards $K$, in other words morphisms into $K$ are a priori filtered. Likewise, the coradical filtration preserves the differential as well. It is clear that $\eta_{\overline{D}}$ preserves this filtration as it acts like the identity.

                The associated graded component reduces to the associated graded of $D$ and $B\Omega D$. This can be seen as if we lower a $n_i$ by $1$, then this component lands in the lower degree of the filtration. By cocontnouity of the tensor, we may then move the associated graded into each variable. The sum handles then every other component.
                \begin{align*}
                ...
                \begin{align*}
                    \widetilde{F}_n(BK\ast\tt{gr}D) & = \bigoplus_{k=0}^\infty\,\,\sum_{\substack{n_1+\cdots +n_k + k \\ \leq n}}\,\,\bigotimes_{i=1}^k(\overline{K}[1]\oplus \tt{gr}_{n_i}\overline{D})\tt{ and} \\
                ...
                Observe that $id_{BK}\ast\tt{gr}(\eta_D)$ preserves this filtration, as it acts like the identity and the first filtration is naturally a subobject of the second filtration by identifying the elements.

                The $0$'th page of the double spectral sequence can be calculated as below.
                \begin{multline*}
                    \widetilde{E}_0^{-p}(BK\ast\tt{gr}D) = \tt{gr}_p(BK\ast\tt{gr}D)[p] \simeq \bigoplus_{k=0}^\infty\,\,\sum_{\substack{n_1+\cdots + n_k + k \\ = p}}\,\,\bigotimes_{j=1}^k\,(\overline{K}[1]\oplus\tt{gr}n_i\overline{D})[p] \\ \simeq \bigoplus_{k=0}^\infty\,(\overline{K}[1])^{\otimes k}\oplus\,\sum_{\substack{n_1+\cdots + n_k + k \\ = p}}\,\,\bigotimes_{j=1}^k\,(\tt{gr}n_i\overline{D})[p] \simeq BK[p] \oplus \bigoplus_{k=0}^\infty\,\,\sum_{\substack{n_1+\cdots + n_k + k \\ = p}}\,\,\bigotimes_{j=1}^k\,(\tt{gr}n_i\overline{D})[p]
                \end{multline*}
                \begin{multline*}
                    \widetilde{E}_0^{-p}(BK\ast B\Omega\tt{gr}D) = \tt{gr}_p(BK\ast B\Omega\tt{gr}D)[p] \\ \simeq \bigoplus_{k=0}^\infty\,\,\sum_{\substack{n_1+\cdots + n_k \\ = n}}\,\,\bigotimes_{i=1}^k\,(\overline{K}[1]\oplus(\bigoplus_{t=1}^\infty\,\,\sum_{\substack{m_1+\cdots + m_t + t \\ = n_i}}\,\,\bigotimes_{j=1}^t\,\tt{gr}_{m_j}\overline{D}[-1])[1])[p] \\ \simeq \bigoplus_{k=0}^\infty\,((\overline{K}[1])^{\otimes k}\oplus\,\sum_{\substack{n_1+\cdots + n_k \\ = n}}\,\,\bigotimes_{i=1}^k\,\bigoplus_{t=1}^\infty\,\,\sum_{\substack{m_1+\cdots + m_t + t \\ = n_i}}\,\,\bigotimes_{j=1}^t\,(\tt{gr}_{m_j}\overline{D}[-1])[1])[p] \\ \simeq BK[p] \oplus (\bigoplus_{k=0}^\infty\,\,\sum_{\substack{n_1+\cdots + n_k \\ = n}}\,\,\bigotimes_{i=1}^k\,\bigoplus_{t=1}^\infty\,\,\sum_{\substack{m_1+\cdots + m_t + t \\ = n_i}}\,\,\bigotimes_{j=1}^t\,(\tt{gr}_{m_j}\overline{D}[-1])[1])[p]
                \end{multline*}

                Here we have been able to split the $0$'th page into a $BK$ part and a $D$ part. This is in fact a splitting of the chain complexes, as the differential coming from $d'$ is of negative filtered degree. In this way $\tt{gr}(id_{BK}\ast\tt{gr}\eta_D)$ is in fact a quasi-isomorphism between the sequences $\widetilde{E}(BK\ast\tt{gr}D) \rightarrow \widetilde{E}(BK\ast B\Omega\tt{gr}D)$. By the classical convergence theorem this assembles into a quasi-isomorphism on the $E_1$ page of the previous spectral sequences, showing that $\pi$ is a filtered quasi-isomorphism.
            \end{proof}
