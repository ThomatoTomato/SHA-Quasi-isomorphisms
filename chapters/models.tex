...
\begin{document}
    ...
    \subsection{A Model Structure on DG-Coalgebras}
            ...
            \begin{proof}
                ...
                By definition, the $0$'th page is defined as $E^0_{p,q}\Omega C = \sfrac{(F_p\Omega C)_{p+q}}{(F_{p-1}\Omega C)_{p+q}}$. Furthermore, notice that at this page we have the following isomorphism $E^0_{p,q}\Omega C \simeq (\Omega grC)^{(p)}_{p+q}$, where $(\Omega grC)^{(p)} = \startset{c_1[-1]\otimes ... \otimes c_n[-1]\mid |c_1|+...+|c_n| = p}$.
            ...
            Let $A$ ($C$) be a filtered dg-algebra (coalgebra). Given an element $a\in A$ ($c\in C$) we say that its filtered degree $fdeg(a)$ ($fdeg(c)$) is the number such that $a\in F_{fdeg(a)}A$ ($c\in F_{fdeg(c)}C$) but not $a\in F_{fdeg(a)+1}A$ ($c\in F_{fdeg(c)+1}C$). There is then an associated filtration on the bar (cobar) construction of this complex, defined as $F_pBA = \startset{a_1 \otimes ... \otimes a_n \mid \sum fdeg(a_i)\leq p}$ ($F_p\Omega C = \startset{ c_1 \otimes ... c_n \mid \sum fdeg(c_i) \leq p}$).
            ...
            \begin{proof}
                ...
                \begin{align*}                    
                    r(s^{-1}(sa)\otimes s^{-1}(...)\otimes ...) = (-1)^{|a|+1}s^{-1}(sa \otimes ...) \otimes ...
                \end{align*}
                We will show that this is a homotopy by induction on $i$.

                Let $i=2$. Then there are two cases we must handle, either an element is on the form $s^{-1}(sa_1)\otimes s^{-1}(sa_2)$ or $s^{-1}(sa_1 \otimes sa_2)$. We consider the latter case first. If we apply $r$ to this element we are returned $0$. 
                \begin{align*}
                    (r\circ d_{\Omega BA}+d_{\Omega BA}\circ r)(s^{-1}(sa_1\otimes sa_2)) = r ((-1)^{|a_1|+1}s^{-1}(sa_1)\otimes s^{-1}(sa_2)) = s^{-1}(sa_1 \otimes sa_2)
                \end{align*}
                The we treat the former case
                \begin{multline*}
                    (r\circ d_{\Omega BA}+d_{\Omega BA}\circ r)(s^{-1}(sa_1)\otimes s^{-1}(sa_2)) \\ 
                    = r(s^{-1}(sd_Aa_1)\otimes s^{-1}(sa) + (-1)^{|a_1|}s^{-1}(sa_1)\otimes s^{-1}(sd_Aa_2)) + d_{\Omega BA}((-1)^{|a_1|+1}s^{-1}(sa_1 \otimes sa_2)) \\
                    = (-1)^{|a_1|}s^{-1}(sd_Aa_1 \otimes sa_2) - s^{-1}(sa_1 \otimes sd_Aa_2) + s^{-1}(sa_1) \otimes s^{-1}(sa_2) \\
                    + (-1)^{|a|+1}s^{-1}(sd_Aa_1 \otimes sa_2) + s^{-1}(sa_1 \otimes sd_Aa_2) = s^{-1}(sa_1) \otimes s^{-1}(sa_2)
                \end{multline*}
                ...
                \begin{align*}
                    F_pB\Omega C = \startset{(c_{1,1}[-1]\otimes ... c_{1,n_1}[-1])) \otimes ... \otimes (c_{m, 1}[-1]\otimes ... \otimes c_{m,n_m}[-1])\mid |c_{1,1}| + ... + |c_{m,n_m}|\leq p}\text{.}
                \end{align*}
                ...
                \begin{align*}
                    G_kEF^0_{p,\bullet} = \startset{(...)_1 \otimes ... \otimes (...)_n \mid n \geq -k}
                \end{align*}
