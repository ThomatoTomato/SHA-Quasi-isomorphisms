...
\begin{document}
    ...
    \subsection{A Model Structure on DG-Coalgebras}
            ...
            \begin{proof}
                ...
                There is a homotopy of the identity given as $h : gr_i \Omega BA \rightarrow gr_i \Omega BA$ as $0$, unless there is an element on the form $s^{-1}(sa)\otimes s^{-1}(...) \otimes ...$, then 
                \begin{align*}                    
                ...
                To extend this argument by induction, we will hypothesize that the terms where the differential is applied will have opposite signs, such that they cancell. Then the result follows for any $i$, by observing that the tensors far enough out to the right is not affected by $r$.

                If $C$ is a dg-coalgebra, we use the same technique. Here we use the coradical filtration of $C$ and a double induced filtration on $B\Omega C$. The unit is a filtered morphism, and we use the same technique to show that it is a quasi-isomorphism on the associated graded.
            \end{proof}
