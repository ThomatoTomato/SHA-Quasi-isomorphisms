...
\begin{document}
    ...
    \subsection{A Model Structure on DG-Coalgebras}
            ...
            To see that these classes of morphisms does indeed define a model structure, we will get a better description for a subclass of the weak equivalences. This can only be done by calculating homologies, since $f$ is a weak equivalence if and only if $H^*\tt{cone}(\Omega f) \simeq 0$. Using spectral sequences to calculate these homologies is not crucial, but it gives us a method to tackle the kinds of problems which we will face.
            ...
            \begin{proof}
                ...
                Since $C$ is a dg-coalgebra, the coradical filtration respects the differential. In other words, $F_p\Omega C$ is still a chain complex, which is a subcomplex of $\Omega C$. This filtration is clearly bounded below and exhaustive. Thus by the classical convergence theorem of spectral sequences, \cite[Theorem 5.5.1][135]{Weibel94}, the spectral sequence converges to the homology $E\Omega C \Rightarrow H^*\Omega C$.
                ...
                Evaluating $f$ at the $0$'th page would look like $E^0\Omega f \simeq \Omega \tt{gr}f$. By the mapping lemma, \cite[Exercise 5.2.3][125]{Weibel94}\todo{Flytt dette til appendix?}, it is enough to check that $\Omega \tt{gr}f$ is a quasi-isomorphism to see that $\Omega f$ is a quasi-isomorphism. We show that $\Omega \tt{gr}f$ is a quasi-isomorphism by inspecting every cochain complex $E^0_{p,\bullet}\Omega C$.
            ...
            This proof will serve as a template for how we want to approach many of the proofs we will encounter. The lemma says that to show that $f$ is a weak equivalence it is sufficient to show that $f$ is a filtered quasi-isomorphism. However, to show that $f$ is a filtered quasi-isomorphism we first need a good filtering, and once we have a filtering we look at its spectral sequence. If we still have a problem, we look at complexes within a page on a spectral sequence, and the mapping lemma says that it is enough to verify that a morphism becomes a quasi-isomorphism on any page to see that it is a quasi-isomorphism. On these complexes we may define new filtrations in order to calculate the next page. We will informally call this technique for an iterated spectral sequence argument.
            ...
            \begin{proof}
                Notice that the homology of $BA$ may calculated from the double complex used to define $BA$. In fact, at the $0$'th page of the canonical spectral sequence we have $E^0_{p, \bullet}f \simeq f^{\otimes p}$. It follows that $f$ is a quasi-isomorphism on the $0$'th page from the Kunneth formula, \cite[Theorem 3.6.3][88]{Weibel94}.            
            \end{proof}
            ...
            \begin{proposition}\label{prop: unit-counit-qif}
                Let $A$ be an augmented dg-algebra and $C$ a conilpotent dg-coalgebra. The counit $\varepsilon_A : \Omega BA \rightarrow A$ a quasi-isomorphism. The unit $\eta_C : C \rightarrow B\Omega C$ is a filtered quasi-isomorphism, moreover $\Omega \eta_C$ is a quasi-isomorphism.
            \end{proposition}
            ...
            \begin{proof}
               We start by showing that the counit is a quasi-isomorphism. Define the following filtration for $A$.
                ...
                To extend this argument by induction, we will observe that the terms where the differential is applied will have opposite signs, such that they cancel. Then the result follows for any $i$, by observing that the tensors far enough out to the right is not affected by $r$.
            ...
            This proof has a troubled past. In \cite{LefevreHasegawa03}, Lefevre-Hasegawa made a proof which was a straightforward modification of Hinich's proof from \cite{Hinich01}. However, this translation does not behave as well as one would like. Keller points out that this method may work in some cases, but fails in its full generality \cite{Keller06}. The proof presented here will be a modification of Vallette's proof of "A technical lemma" \cite[Appendix B]{Vallette20}.

            \begin{proof}
                $\pi$ being a cofibration is immediate by Corollary~\ref{cor: stable-cofib-base-change}. 
                
                To see that $\pi$ is a weak equivalence it is enough to show that it is a filtered quasi-isomorphism by Lemma~\ref{lem: graded-qif-are-w}. Since we assume $p$ to be a fibration onto a quasi-free algebra we may realize the algebra $A$ as the following extension.
                \begin{center}
                    \begin{tikzcd}
                ...
                In the category $\tt{Alg}_{\mathbb{K},+}^\bullet$, $\oplus$ is the product. Since $B : \tt{Alg}_{\mathbb{K},+}^\bullet \rightarrow \tt{coAlg}_{\mathbb{K}, conil}^\bullet$ is right adjoint, it necessarilly preserves products. Thus
                \begin{align*}
                ...
                Since $BK$ is quasi-free, by the comonadic presentation of $D$, we are able to obtain an identification of graded modules, $BK\ast D \subseteq T^c(\overline{K}[1]\oplus \overline{D})$. Likewise, since both $BK$ and $B\Omega D$ are quasi-free, we realize the product as $BK\ast B\Omega D \simeq T^c(\overline{K}[1]\oplus (\overline{\Omega}D)[1])$.
                ...
                The associated graded component reduces to the associated graded of $D$ and $B\Omega D$. This can be seen as if we lower the degree of a $n_i$ by $1$, then this component lands in the lower degree of the filtration. By cocontinouity of the tensor, we may then move the associated graded into each variable. The sum then handles every other component.
                \begin{align*}
            ...
            \begin{proof}
                ...
                We now show \textbf{MC3}. Suppose that there is a square as below, where $i$ is a cofibration and $t$ is an acyclic cofibration.
                \begin{center}
                    \begin{tikzcd}
    ...
    \subsection{Homotopy theory of $A_{\infty}$-algebras}
        ...
        We will first prove one lemma which technique we will reuse. A second lemma used to simplify the theorem will be stated without proof.
        ...
        \begin{proof}
            The proof may be found in \cite[Lemma 1.3.3.3][44]{LefevreHasegawa03}.
        \end{proof}
        ...
        \begin{proof}[Proof of Theorem~\ref{thm: model-A-inf}]
            ...
            Since $g_1$ is an epimorphism, $A[1]$ as a graded vector space splits into $A''[1] \oplus K$, where $K = \tt{Ker}g_1$. The pullback is then naturally identified with $BA \prod_{BA''}BA' \simeq \overline{T}^c(K)\prod \overline{T}^c(A'[1])$ as graded vector spaces. Since the cofree coalgebra is right adjoint to forget, it commutes with products and we get, $\overline{T}^c(A'[1])\prod \overline{T}^c(K) \simeq \overline{T}^c(A'[1]\oplus K)$. Thus the pullback is isomorphic to a cofree coalgebra as a graded coalgebra, i.e. it is an $A_\infty$-algebra.
        ...
        \begin{proof}
            Suppose that $f: A \rightarrow A'$ is an $\infty$-quasi-isomorphism. By the Kunneth theorem, $Bf : BA \rightarrow BA'$ is a filtered quasi-isomorphism.
            ...
            The cofibrations of $\tt{coAlg}_{\mathbb{K},conil}^\bullet$ are monomorphisms. Since $B$ is an equivalence of categories it necessarilly have to preserve and reflect monomorphisms. \todo{Er virkelig monomorfier av A-uendelig-algebraer morfier hvor $f_1$ er mono?}
